...
try:
	while True:
		...
		if (msg.arbitration_id == US1) :
			...
			distance1 = int.from_bytes(msg.data[0:2], byteorder='big')
			message1 = "URL:" + str(distance1)+ ";"
			print(message1)
			if distance1 > 50:
				msg = can.Message(arbitration_id=0x010,data=[0xBA,0xBA,0x00, 0x00, 0x00, 0x00,0x00, 0x00],extended_id=False)
				bus.send(msg)
			...
			distance2 = int.from_bytes(msg.data[2:4], byteorder='big')
			message2 = "URR:" + str(distance2)+ ";"
			print(message2)
			if distance2 > 50:
				msg = can.Message(arbitration_id=0x010,data=[0xBA,0xBA,0x00, 0x00, 0x00, 0x00,0x00, 0x00],extended_id=False)
				bus.send(msg)
